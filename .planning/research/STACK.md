...
**Domain:** Python image processing microservice (background removal + standardization)
**Researched:** 2026-03-30
**Confidence:** HIGH — todas las versiones verificadas contra PyPI oficial
...
**Nota crítica sobre rembg[cpu]:** El extra `[cpu]` instala `onnxruntime` (CPU-only) y previene que pip intente instalar `onnxruntime-gpu`. En el Dockerfile esto es obligatorio dado que el VPS no tiene GPU.
...
**Si el volumen aumenta a > 500 imgs/día:**
...
**Si se agrega soporte GPU en el futuro:**
...
**Si se necesitan múltiples formatos de salida (v2):**
...
**Rationale:** Descarga del modelo en build time es crítica. Sin esto, el primer request tarda 1-3 minutos y puede timeout en el caller (n8n). El modelo birefnet-lite pesa ~300MB y se almacena en la imagen, que resultará en ~1.2-1.5GB final — aceptable para este caso de uso.
...
*Stack research for: Python image processing microservice (rembg + FastAPI + Docker)*
*Researched: 2026-03-30*
